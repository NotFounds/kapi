...
void encode_symbols(FILE *out)
{
    ...
    for (i = 0; i < end_of_symbols; ++i)
    {
        ...
        {
            error(ERROR_INTERNAL, "encode_symols", 0);
        }
    }
}
...
{
    int i;
    int fp = end_of_symbols - find_symbols("fp") - 1;

    for (i = 0; i < end_of_symbols; ++i)
    {
        if (symbols[end_of_symbols - i - 1].level != 0)
        {
            symbols[end_of_symbols - i - 1].offset = 4 * (i - fp) - symbols[end_of_symbols - i - 1].size + 4;
        }
    }
}
