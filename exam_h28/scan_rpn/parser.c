...
void parse_function()
{
	...
	while(token==TOKEN_BYTE || token==TOKEN_LONG || token==TOKEN_WORD) {
		parse_variable();
...
{
	gen_code("start", "main");
	while(token==TOKEN_BYTE || token==TOKEN_LONG || token==TOKEN_WORD ||token==TOKEN_FUNCTION){
		if(token==TOKEN_FUNCTION){
			parse_function();
		} else {
			parse_variable();
